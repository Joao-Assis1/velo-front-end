...
test.describe('Stripe Connect Onboarding - Instrutor', () => {
  test('Verifica seção Conta de Recebimento e banner financeiro', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'seu@email.com' }).fill('instrutor.teste@velo.com');
    ...
    // 8. Verify "Conta de Recebimento" section is visible
    await expect(page.getByText('Conta de Recebimento')).toBeVisible();

    // 9. Verify ONBOARDING state — "Cadastro em análise pelo Stripe"
    await expect(page.getByText('Cadastro em análise pelo Stripe')).toBeVisible();

    // 10. Verify "Continuar cadastro no Stripe" action button is visible
    await expect(page.getByRole('button', { name: 'Continuar cadastro no Stripe' })).toBeVisible();
    ...
    // 14. Verify finance banner — "Conta de recebimento não configurada"
    await expect(page.getByText('Conta de recebimento não configurada')).toBeVisible();

    // 15. Verify banner description text
    await expect(page.getByText('Conecte sua conta bancária para receber os pagamentos das suas aulas.')).toBeVisible();
    ...
    await expect(configurarLink).toHaveAttribute('href', '/app/instructor/settings');
  });
});
