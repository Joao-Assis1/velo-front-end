...
test.describe("Settings — Stripe Connect", () => {
  ...
  test("exibe heading e seção chave PIX", async ({ page }) => {
    await expect(
  ...
  test("estado PENDING — botão conectar conta visível após resolver loading", async ({
    page,
  }) => {
    // Aguarda o skeleton desaparecer: API retorna erro com token fake → estado PENDING
    const connectBtn = page.getByRole("button", {
      name: /conectar conta bancária via stripe/i,
    });
  ...
  test("estado PENDING — seção senha coexiste com seção connect", async ({
    page,
  }) => {
    // Aguarda o connect resolver antes de checar os outros elementos
    await expect(
      page.getByRole("button", { name: /conectar conta bancária via stripe/i }),
    ).toBeVisible({ timeout: BACKEND_TIMEOUT });
    await expect(
      page.getByRole("button", { name: /redefinir senha/i }),
    ).toBeVisible();
  });
